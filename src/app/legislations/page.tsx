...
const CategoryBadge = ({ category }: { category: string }) => {
  const map: Record<string, string> = { "Aviation": "#1B2A6B", "Maritime": "#0077B6", "Railways": "#6A0572", "General": "#2d6a4f" };
  return (
    <span style={{ display: "inline-block", padding: "0.2rem 0.55rem", borderRadius: "10px", fontSize: "0.66rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em", backgroundColor: map[category] ?? "#64748B", color: "#fff", whiteSpace: "nowrap" }}>{category}</span>
  );
};

const DownloadBtn = () => (
  <button onClick={(e) => { e.stopPropagation(); }} style={{ display: "inline-flex", alignItems: "center", gap: "0.35rem", padding: "0.35rem 0.8rem", backgroundColor: "#E23030", color: "white", border: "none", borderRadius: "6px", fontSize: "0.72rem", fontWeight: 700, cursor: "pointer", transition: "all 0.18s ease", whiteSpace: "nowrap" }}
...
const TH = ({ children, width }: { children: React.ReactNode; width?: string }) => (
  <th style={{ padding: "0.85rem 1rem", fontSize: "0.72rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: "rgba(255,255,255,0.95)", borderBottom: "2px solid rgba(255,255,255,0.15)", textAlign: "left", whiteSpace: "nowrap", width: width || "auto" }}>{children}</th>
);
const TD = ({ children, muted }: { children: React.ReactNode; muted?: boolean }) => (
  <td style={{ padding: "0.9rem 1rem", fontSize: "0.855rem", color: muted ? "#6A6B70" : "#1E293B", borderBottom: "1px solid #F1F5F9", verticalAlign: "middle" }}>{children}</td>
);
...
const legislationRecords: LegRecord[] = [
  { sn: 1,  dateEnacted: "2026-01-15", refNo: "LEG/001/2026", title: "NSIB Establishment Act 2022 (Amended 2026)",                    category: "General",   status: "In Force" },
  { sn: 2,  dateEnacted: "2025-11-20", refNo: "LEG/002/2025", title: "Civil Aviation (Amendment) Act 2025",                           category: "Aviation",  status: "In Force" },
  { sn: 3,  dateEnacted: "2025-09-10", refNo: "LEG/003/2025", title: "NSIB Safety Investigation Regulations 2025",                    category: "General",   status: "In Force" },
  { sn: 4,  dateEnacted: "2025-08-05", refNo: "LEG/004/2025", title: "Maritime Operations Safety Act (Amendment) 2025",                category: "Maritime",  status: "In Force" },
  { sn: 5,  dateEnacted: "2025-06-28", refNo: "LEG/005/2025", title: "Railway Safety Regulations 2025",                               category: "Railways",  status: "In Force" },
  { sn: 6,  dateEnacted: "2025-05-15", refNo: "LEG/006/2025", title: "Transport Accident Investigation (Procedures) Regulations",     category: "General",   status: "In Force" },
  { sn: 7,  dateEnacted: "2025-04-02", refNo: "LEG/007/2025", title: "Nigerian Airspace Management Agency Act (Consolidated)",        category: "Aviation",  status: "In Force" },
  { sn: 8,  dateEnacted: "2025-02-18", refNo: "LEG/008/2025", title: "Cabotage Act (Nigeria Coastal and Inland Shipping)",            category: "Maritime",  status: "In Force" },
  { sn: 9,  dateEnacted: "2024-12-10", refNo: "LEG/009/2024", title: "NSIB (Staff Conditions & Benefits) Regulations 2024",           category: "General",   status: "In Force" },
  { sn: 10, dateEnacted: "2024-11-05", refNo: "LEG/010/2024", title: "Civil Aviation Act 2006 (Original)",                            category: "Aviation",  status: "Amended" },
  { sn: 11, dateEnacted: "2024-09-22", refNo: "LEG/011/2024", title: "Merchant Shipping Act",                                         category: "Maritime",  status: "In Force" },
  { sn: 12, dateEnacted: "2024-08-14", refNo: "LEG/012/2024", title: "Nigerian Railway Corporation Act (Cap N129 LFN 2004)",          category: "Railways",  status: "In Force" },
  { sn: 13, dateEnacted: "2024-07-01", refNo: "LEG/013/2024", title: "Federal Airports Authority of Nigeria Act",                     category: "Aviation",  status: "In Force" },
  { sn: 14, dateEnacted: "2024-06-10", refNo: "LEG/014/2024", title: "International Maritime Organization Conventions (Ratification)",category: "Maritime",  status: "In Force" },
  { sn: 15, dateEnacted: "2024-05-05", refNo: "LEG/015/2024", title: "Accident Investigation (Confidentiality) Regulations 2024",     category: "General",   status: "In Force" },
  { sn: 16, dateEnacted: "2024-04-18", refNo: "LEG/016/2024", title: "Nigerian Civil Aviation Regulations (Part 13 - Investigation)", category: "Aviation",  status: "In Force" },
  { sn: 17, dateEnacted: "2024-03-02", refNo: "LEG/017/2024", title: "NIMASA Establishment Act 2007",                                 category: "Maritime",  status: "Amended" },
  { sn: 18, dateEnacted: "2024-02-10", refNo: "LEG/018/2024", title: "Railway Infrastructure (Safety Standards) Order 2024",           category: "Railways",  status: "In Force" },
  { sn: 19, dateEnacted: "2024-01-20", refNo: "LEG/019/2024", title: "Freedom of Information Act 2011 (as applicable)",               category: "General",   status: "In Force" },
  { sn: 20, dateEnacted: "2023-12-05", refNo: "LEG/020/2023", title: "NSIB Regulations 2023 (Original)",                              category: "General",   status: "Amended" },
];
...
export default function LegislationsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return legislationRecords
      .filter(r => {
        const matchSearch = !q || r.title.toLowerCase().includes(q) || r.refNo.toLowerCase().includes(q) || r.category.toLowerCase().includes(q);
        const matchYear = !yearFilter || r.dateEnacted.startsWith(yearFilter);
  ...
  const StatusBadge = ({ status }: { status: string }) => {
    const map: Record<string, { bg: string; text: string }> = { "In Force": { bg: "#1B2A6B", text: "#fff" }, "Amended": { bg: "#D97706", text: "#fff" }, "Repealed": { bg: "#E23030", text: "#fff" } };
    const c = map[status] ?? { bg: "#64748B", text: "#fff" };
    return (<span style={{ display: "inline-block", padding: "0.2rem 0.65rem", borderRadius: "10px", fontSize: "0.68rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em", backgroundColor: c.bg, color: c.text, whiteSpace: "nowrap" }}>{status}</span>);
  };
        ...
        <motion.div className="container" style={{ position: "relative", zIndex: 2, opacity: opacityFade }} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: "easeOut" }}>
          <div style={{ maxWidth: "800px" }}>
            <motion.h1 initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2 }}
              ...
              Acts &amp;<br/>Legislations
            </motion.h1>
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.4 }}
              style={{ fontSize: "1.2rem", color: "rgba(255,255,255,0.85)", maxWidth: "600px", lineHeight: 1.7 }}>
      ...
      <div className="container" style={{ maxWidth: "1400px" }}>
        ...
        <div style={{ display: "flex", gap: "1rem", marginBottom: "1.5rem", flexWrap: "wrap", alignItems: "center" }}>
          <div style={{ position: "relative", flex: "1 1 260px", maxWidth: "380px" }}>
            ...
            <input type="text" placeholder="Search by title, ref no, category…" value={search} onChange={e => handleSearch(e.target.value)}
          ...
          <select value={yearFilter} onChange={e => handleYear(e.target.value)} style={{ padding: "0.7rem 1rem", border: "1.5px solid #E2E8F0", borderRadius: "8px", fontSize: "0.875rem", color: "#1E293B", backgroundColor: "white", cursor: "pointer", outline: "none" }}>
        ...
        <div style={{ backgroundColor: "white", borderRadius: "12px", border: "1px solid #E2E8F0", overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", minWidth: "900px" }}>
              <thead><tr style={{ backgroundColor: "#1B2A6B" }}>
                <TH width="52px">S/N</TH>
                <TH width="110px">Date Enacted</TH>
                <TH width="140px">Ref No.</TH>
                <TH>Title</TH>
              ...
              <tbody>
                {paged.length > 0 ? paged.map((r, idx) => {
                  ...
                  return (
                    <tr key={r.sn} style={{ backgroundColor: rowBg, transition: "background 0.15s", cursor: "default" }}
                      ...
                      <TD muted>{formatDate(r.dateEnacted)}</TD>
                      <td style={{ padding: "0.9rem 1rem", verticalAlign: "middle", borderBottom: "1px solid #F1F5F9" }}><span style={{ fontFamily: "monospace", fontSize: "0.8rem", fontWeight: 700, color: "#1B2A6B" }}>{r.refNo}</span></td>
                      <TD>{r.title}</TD>
                ...
                }) : (<tr><td colSpan={7} style={{ padding: "4rem 2rem", textAlign: "center", color: "#94A3B8" }}>No records match your search criteria.</td></tr>)}
              </tbody>
            </table>
          </div>
          {totalPages > 1 && (
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "1rem 1.5rem", borderTop: "1px solid #F1F5F9", backgroundColor: "white", flexWrap: "wrap", gap: "1rem" }}>
              ...
              <div style={{ display: "flex", gap: "0.4rem", alignItems: "center" }}>
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).map(n => (<button key={n} onClick={() => setPage(n)} style={{ width: "36px", height: "36px", borderRadius: "8px", border: n === page ? "1.5px solid #1B2A6B" : "1.5px solid #E2E8F0", backgroundColor: n === page ? "#1B2A6B" : "white", color: n === page ? "white" : "#1B2A6B", fontSize: "0.85rem", fontWeight: n === page ? 700 : 500, cursor: "pointer", transition: "all 0.15s" }}>{n}</button>))}
        ...
        <div style={{ display: "flex", gap: "1.5rem", marginTop: "1.5rem", flexWrap: "wrap", alignItems: "center" }}>
          ...
          {[["In Force", "#1B2A6B"], ["Amended", "#D97706"], ["Repealed", "#E23030"]].map(([label, bg]) => (
